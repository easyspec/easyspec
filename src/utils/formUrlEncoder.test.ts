...
describe("formUrlEncoder", () => {
  ...
  describe("isURLSafe", () => {
    ...
    it("should return true for URLs under 2000 characters", () => {
      const url = "https://example.com/" + "a".repeat(1900);
      expect(isURLSafe(url)).toBe(true);
    });

    it("should return true for URLs exactly 2000 characters", () => {
      const url = "a".repeat(2000);
      expect(isURLSafe(url)).toBe(true);
    });

    it("should return false for URLs over 2000 characters", () => {
      const url = "a".repeat(2001);
